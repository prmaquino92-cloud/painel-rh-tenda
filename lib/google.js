...
// Envia um e-mail pela conta Google conectada (mesma usada pra Agenda) via Gmail API.
// Best-effort: retorna false (sem lançar erro) se o Google não estiver conectado. Se o escopo
// gmail.send ainda não tiver sido concedido (conexão feita antes desse recurso existir), a API
// responde com erro de permissão — nesse caso é preciso desconectar e reconectar o Google em
// Integrações para autorizar o novo escopo.
export async function enviarEmail({ to, toName, subject, html }) {
  if (!to) return false;
  const accessToken = await getValidAccessToken();
  if (!accessToken) return false;

  const destinatario = toName ? `${toName} <${to}>` : to;
  const assuntoCodificado = `=?UTF-8?B?${Buffer.from(subject || '', 'utf-8').toString('base64')}?=`;
  const mensagem = [
    `To: ${destinatario}`,
  ...
  const r = await fetch(GMAIL_SEND_URL, {
    ...
    body: JSON.stringify({ raw: base64UrlEncode(mensagem) }),
  });
  if (!r.ok) {
    const json = await r.json().catch(() => null);
    throw new Error(json?.error?.message || 'Falha ao enviar e-mail pelo Gmail');
  }
  return true;
}
