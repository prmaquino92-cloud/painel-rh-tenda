...
// Link único e genérico de candidatura — não pertence a nenhuma vaga específica. Pedro define
// depois, pelo painel, para qual vaga cada candidato que entrar por aqui vai. Por isso os campos
// do formulário e a janela de agendamento usam uma configuração padrão (não a de uma vaga).
const CAMPOS_PADRAO = { idade: true, localizacao: true, telefone: true, email: true, redes: true, curriculo: true };

export default function CandidaturaGeral() {
  ...
  useEffect(() => {
    if (step === 2 && !dias && !carregandoSlots) {
      setCarregandoSlots(true);
      fetch('/api/public/slots-geral')
        .then((r) => r.json())
  ...
  async function confirmar() {
    ...
    setErro('');
    try {
      const r = await fetch('/api/public/candidatura-geral', {
        method: 'POST',
      ...
      <PublicShell titulo="Entrevista agendada">
        <div className="public-card" style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--ink-soft)', margin: '0 0 18px' }}>
            Sua entrevista com a Tenda Vendas foi marcada para <b>{slotLabel}</b>.
          </p>
  ...
  const campos = CAMPOS_PADRAO;

  return (
    <PublicShell titulo="Trabalhe conosco">
      ...
      <div className="public-card">
        <div className="kicker">Tenda Vendas · Trabalhe conosco</div>
        <h1 style={{ fontSize: 21, margin: '8px 0 4px' }}>Cadastre sua candidatura</h1>
        <p style={{ fontSize: 13, color: 'var(--ink-soft)', margin: '0 0 18px' }}>
          Preencha seus dados e escolha um horário para conversarmos. Depois definimos juntos a vaga mais adequada.
        </p>
